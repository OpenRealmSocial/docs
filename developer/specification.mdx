...
Three distinct contracts manage the tracking of account identifiers (fids), their associated keys, and allocated storage space.

## Identity Registry

The Identity Registry contract is responsible for maintaining a record of fids along with their corresponding custody addresses. It operates as a straightforward mapping from each fid to its custody address. An fid is considered valid only when listed in the Identity Registry.

The official Identity Registry contract can be found at /*needs to be added*/

## Key Registry

This contract manages the approved signing keys for fids. A signer is deemed legitimate for an fid if they are registered in the Signature Registry specific to that fid. Only the fid's custody address has the authority to add or remove signers.

The official Signature Registry contract is available at *needs to be added*\

## Storage Registry

This contract keeps a log of the storage space allotted to each fid, quantified in integer units. Each CRDT defines its storage capacity per unit.
...
The server which resolves `*.fcast.id` names lives at `fnames.openrealm.social`. Fnames can be claimed by submitting an EIP-712 signed message that proves ownership of an fid that does not yet have an fname. The server also provides a method to transfer fnames to other fids by proving ownership of the fname.

Usernames are also valid subdomains (e.g. [foo.fcast.id.openrealm.social] ) though they do not currently resolve to anything. A future upgrade to the nameserver may allow the owner to set a redirect record here. The following usernames are not available for registration, since they collide with existing subdomains — `www`, `fnames`
