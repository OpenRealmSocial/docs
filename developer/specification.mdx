...
## Requirements to implement a functional version of the OpenRealm Protocol

Version: `v0.1.0`

# Smart Contracts

Three distinct contracts manage the tracking of account identifiers (qids), their associated keys, and allocated storage space.

## Identity Registry

The Identity Registry contract is responsible for maintaining a record of qids along with their corresponding custody addresses. It operates as a straightforward mapping from each qid to its custody address. An qid is considered valid only when listed in the Identity Registry.

The official Identity Registry contract can be found at /*needs to be added*/

## Key Registry

This contract manages the approved signing keys for qids. A signer is deemed legitimate for an qid if they are registered in the Signature Registry specific to that qid. Only the qid's custody address has the authority to add or remove signers.

The official Signature Registry contract is available at *needs to be added*\

## Storage Registry

This contract keeps a log of the storage space allotted to each qid, quantified in integer units. Each CRDT defines its storage capacity per unit.

You can view the official Allocation Registry contract at *needs to be added*

# Message Specification

A Message in the OpenRealm network is a binary data unit, cryptographically signed, representing a change-operation. Messages are structured and converted into binary format employing proto3 protobufs.
...
6. `signer` is a valid public key or Ethereum address used to produce the sig
...
- `EIP712`: A 512-bit [EIP-712](https://eips.ethereum.org/EIPS/eip-712) typed data with a OpenRealm domain separator.
...
```
# OpenRealm Domain Separator

```json
{
  "name": " Verify Ethereum Address",
  "version": "2.0.0",
  "salt": "0x9a4c83be2d1f56a8e9b3f074edc123ab56e7f9c834ba21de8f45a6bc9123d4e8"
}
```
...
A `MessageData` object contains generic properties like the `qid`, `network` `timestamp` and `type` along with a `body`, which varies based on the `type`.
...
2. `m.data.qid` must be an integer >= 0.
3. `m.data.timestamp` must be a valid OpenRealm epoch timestamp not more than 600 seconds ahead of the current time.
...
Timestamps must be seconds since the OpenRealm epoch, which began on Jan 1, 2021 00:00:00 UTC.
...
```protobuf
enum OpenRealmNetwork {
  OPENREALM_NETWORK_NONE = 0;
  OPENREALM_NETWORK_MAINNET = 1; // Public, stable primary network
  OPENREALM_NETWORK_TESTNET = 2; // Public, stable test network
  OPENREALM_NETWORK_DEVNET = 3;  // Public, unstable test network
}
```
...
A user authorizes an application's Signer with a signature from their custody address currently holding their qid. The application can use the Signer to authorize Casts, Reactions and Verifications for that user. Users can revoke a Signer at any time with a signature from their custody address.

![](/images/signers-chart.png)

A Signer is added or removed by registering the public key of the signer to an qid with a smart contract at a well known address. Signers can only be added for the qid owned by the caller of the contract.
...
8. If `m.data.body.type` is `USER_DATA_TYPE_USERNAME`, value must map to a valid qname.
9. `m.data.body.value` must be a valid utf-8 string

A username is considered valid only if the most recent event for the qid `Transfer` event with the custody address in the `to` property. If a valid username for a given qid becomes invalid, and there is a UserDataAdd message for that qid with the qname as its value, it must be revoked. The underlying username proofs are checked once per day to determine if they are still valid.
...
A cast may mention users, but mentions are stored separately from the text property. A mention is created by adding the user's qid to the `mentions` array and its position in bytes in the text field into the `mentions_positions` array. Casts may have up to 10 mentions. The cast "🤓 @openrealm says hello" would be represented as:
...
message CastAddBody {
  ...
  repeated uint64 mentions = 2;           // User qids mentioned in the text
...
message CastId {
  uint64 qid = 1;                           // qid of the cast's author
...
4. `m.data.body.embeds_deprecated` can contain up to 2 valid UTF8 strings whose lengths are >=1 byte and `<= 256 bytes` if the timestamp is `<= 7361280` (5/3/23 00:00 UTC).
...
struct VerificationClaim {
  BigInt qid;         // qid of the user making the claim
  string address;     // Ethereum address signing the claim
  string network;     // OpenRealm network that the claim is meant for
  string blockHash;   // Blockhash at which the claim was made
}
```

An [EIP-712](https://eips.ethereum.org/EIPS/eip-712) signature is requested from the Ethereum address using the OpenRealm domain separator. A Verification is then added by constructing a `VerificationAdd` message which includes the signature and can be removed with a `VerificationRemove` message.
...
4.  `m.data.body.target` must be a known qid.
...
4. `m.data.body.qid` must be a known qid.

A UsernameProof message `m` of type `USERNAME_TYPE_ENS_Qname` must also pass these validations:

1. `m.data.body.name` name must match the regular expression `/^[a-z0-9][a-z0-9-]{0,15}$/`.
2. `m.data.body.owner` must be the custody address of the qid.
3. `m.data.body.signature` must be a valid ECDSA signature on the EIP-712 Username Proof message from the owner or the public key of the qname server.
...
2. `m.data.body.owner` must:
   1. be the custody address or an address that the qid has a valid VerificationMessage for.
...
1. Messages with an EIP-712 signature scheme are only valid if the signing Ethereum address is the owner of the qid.
2. Messages with an ED25519 signature scheme are only valid if the signing key pair is a Signer is present in the Key registry for the qid and has never been removed.
3. Messages are only valid if the qid is owned by the custody address that signed the message, or the signer of the message, which is specified by the Id Registry.
...
1. When a Signer is removed for an qid from the Key registry, all messages signed by the signer in other CRDTs should be revoked.
...
1. `m.signer` must be a valid Signer in the add-set of the Signer CRDT for `message.qid`

A conflict occurs if two messages have the same values for `m.data.qid` and `m.data.body.type`. Conflicts are resolved with the following rules:
...
A conflict occurs if two messages have the same values for `m.data.qid`, `m.data.body.target` and `m.data.body.type`. Conflicts are resolved with the following rules:
...
A conflict occurs if there are two messages with the same values for `m.data.qid`, `m.data.body.type`, `m.data.body.target`. Conflicts are resolved with the following rules:
...
The UsernameProof CRDT validates and accepts UsernameProof messages. It must also continuously re-validate ownership of the username by running a job at 2am UTC to verify ownership of all qnames and ENS Proofs. The CRDT also ensures that a UsernameProof message m passes these validations:
...
1. If m.data.timestamp values are distinct, discard the message with the lower timestamp.
2. If m.data.timestamp values are identical, discard the message with the lower qid

The UsernameProof CRDT has a per-unit size limit of 5.

# Hub Specifications

A Hub is a node in the OpenRealm network that provides an eventually consistent view of network state.

Hubs monitor OpenRealm contracts on Ethereum to track the state of identities on the network. Hubs also maintain and synchronize CRDTs with other Hub by exchanging messages. Hubs communicate using a gossip protocol as the primary delivery mechanism with an out-of-band sync process to handle edge cases.

# Gossip Specifications
...
service HubService {
  ...
  rpc GetCastsByqid(qidRequest) returns (MessagesResponse);
  rpc GetCastsByParent(CastsByParentRequest) returns (MessagesResponse);
  rpc GetCastsByMention(qidRequest) returns (MessagesResponse);

  // Reactions
  rpc GetReaction(ReactionRequest) returns (Message);
  rpc GetReactionsByqid(ReactionsByqidRequest) returns (MessagesResponse);
  ...
  rpc GetLinksByqid(LinksByqidRequest) returns (MessagesResponse);
  rpc GetLinksByTarget(LinksByTargetRequest) returns (MessagesResponse);
  rpc GetAllLinkMessagesByqid(qidRequest) returns (MessagesResponse);

  // User Data
  rpc GetUserData(UserDataRequest) returns (Message);
  rpc GetUserDataByqid(qidRequest) returns (MessagesResponse);

  // Verifications
  rpc GetVerification(VerificationRequest) returns (Message);
  rpc GetVerificationsByqid(qidRequest) returns (MessagesResponse);

   // OnChain Events
   rpc GetOnChainSigner(SignerRequest) returns (OnChainEvent);
   rpc GetOnChainSignersByqid(qidRequest) returns (OnChainEventResponse);
   rpc GetOnChainEvents(OnChainEventRequest) returns (OnChainEventResponse);
   rpc GetIdRegistryOnChainEvent(qidRequest) returns (OnChainEvent);
   rpc GetIdRegistryOnChainEventByAddress(IdRegistryEventByAddressRequest) returns (OnChainEvent);
   rpc GetCurrentStorageLimitsByqid(qidRequest) returns (StorageLimitsResponse);  rpc Getqids(qidsRequest) returns (qidsResponse);
   rpc Getqids(qidsRequest) returns (qidsResponse);

  // Username Proofs
  rpc GetUserNameProof(UserNameProofRequest) returns (UserNameProof);
  rpc GetUserNameProofsByqid(qidRequest) returns (UserNameProofsResponse);

  // Bulk Methods
  rpc GetAllCastMessagesByqid(qidRequest) returns (MessagesResponse);
  rpc GetAllReactionMessagesByqid(qidRequest) returns (MessagesResponse);
  rpc GetAllVerificationMessagesByqid(qidRequest) returns (MessagesResponse);
  rpc GetAllSignerMessagesByqid(qidRequest) returns (MessagesResponse);
  rpc GetAllUserDataMessagesByqid(qidRequest) returns (MessagesResponse);
}
...
message qidsResponse {
  repeated uint64 qids = 1;
...
# Qname Specifications

# ENS CCIP Contract

A CCIP [ENSIP-10](https://docs.ens.domains/ens-improvement-proposals/ensip-10-wildcard-resolution) contract will be deployed on L1 which resolves \*.qcast.id names to owner addresses. It stores the URL of the nameserver and validates signatures provided by the nameserver. This resolver will support addr record lookups only. The address of the contract is **\_\_** (to be filled on deployment).

# Name Server

The server which resolves `*.qcast.id` names lives at `qnames.openrealm.social`. Qnames can be claimed by submitting an EIP-712 signed message that proves ownership of an qid that does not yet have an qname. The server also provides a method to transfer qnames to other qids by proving ownership of the qname.

Usernames are also valid subdomains (e.g. [foo.qcast.id.openrealm.social] ) though they do not currently resolve to anything. A future upgrade to the nameserver may allow the owner to set a redirect record here. The following usernames are not available for registration, since they collide with existing subdomains — `www`, `qnames`

# Managing Qname Ownership
...
```jsx
{
  "from": <qid>"                   // 0 for registering a new qname
  "to": <qid>                      // 0 for unregistering an existing qname
  "name": "<username>",            // qname
  "timestamp": <current_timestamp> // Second resolution
  "owner": "<address>"             // ETH custody address of the non-zero "from"/"to" qid as of timestamp
...
1. The qname is owned by the “from” qid or is not owned by anyone.
2. The “to” qid does not currently own a username.
3. The name matches the regular expression `/^[a-z0-9][a-z0-9-]{0,15}$/`.
4. The timestamp is ≤ current time + 1 minute (for clock skew).
5. The owner must be
   1. the address that owns the “from” qid, if the “from” qid is not 0.
   2. the address that owns the “to” qid, if the “from” qid is 0.
...
7. If there exists an existing proof for the qid, the timestamp of this message must be `2419200` seconds (28 days) ahead of that timestamp to prevent abuse. i.e. an qid can only change their name once every 28 days

The domain and types for the EIP-712 signature are described below:

```jsx
const domain = {
  name: 'OpenRealm name verification',
  version: '1',
  chainId: 1,
  verifyingContract: '0x931c96bcc44eef87852bcd82b7e4e9fd38b5373adb1719f2ab16ac595a5b8dfe', // name registry contract, will be the OpenRealm ENS CCIP contract later
};
	...
	"transfers": [
		{
			...
			"to": 1,
		         ...
		         "owner": "0x931c96bcc44eef87852bcd82b7e4e9fd38b5373adb1719f2ab16ac595a5b8dfe",
			 // EIP-712 signature signed by the server's key
			 "server_signature": "0x7c01b4c38d51e19d9a3844b536ce651f40a26cfdb6e6f44c76122ca695ad1d0bf3e876930db235c10f107cd53a1544b85e7c0a4ca8ec0d32b57e0b218c8",
			 // Original user provided signature
			 "user_signature": "0xa532c71f402d00c3b9c66123e3ebc4c9e3c3cfd3d57c175a9285e24f36e470b94a74e1633b20fc15a69a5d2f0f4a7ea8c83e3e36ea1967b5eaedd8b1a2",
		},
		// ...
	]
}
...
qid=<qid>           // filter events by a particular qid
...
OpenRealm is a long-lived protocol built on the idea of [stability without stagnation](https://doc.rust-lang.org/1.30.0/book/second-edition/appendix-07-nightly-rust.html). Upgrades are designed to be regular and painless, bringing continual improvements for users and developers.
