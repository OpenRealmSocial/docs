...
Each OpenRealm account signifies an individual on the network, identified by a unique OpenRealm ID (fid), like 78213. These fids are allocated and managed through an Ethereum-based IdRegistry contract. Users obtain a new fid via a transaction with IdRegistry. The fid's owning address is termed the 'custody address.' The IdRegistry guarantees unique fid assignment to addresses and enables fid transfers.

# Usernames

Usernames serve to recognize or reference accounts. An account can link multiple usernames, but only one remains active. OpenRealm accommodates both onchain ENS names and offchain names via its fcast.id domain. Each account can claim a complimentary name under fcast.id, but names from other domains like .eth are also usable. Claiming a username involves submitting a signed proof to the network.
