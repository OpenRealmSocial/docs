...
1. `start` - start hub and configure how it should run
...
```
# if using docker
docker compose exec hub yarn <command> 
...
Hub Options:
  -n --network <network>                ID of the Openrealm Network (default: 3 (devnet))
  -i, --id <filepath>                   Path to the PeerId file.
  --hub-operator-qid <qid>              The QID of the hub operator. Optional.
...
Ethereum Options:
  ...
  --qname-server-url <url>              The URL for the FName registry server (default: https://qnames.openrealm.xyz)

L2 Options:
  --l2-id-registry-address                The address of the L2 Openrealm ID Registry contract
  --l2-key-registry-address <address>     The address of the L2 Openrealm Key Registry contract
  --l2-id-registry-v2-address             The address of the L2 Openrealm ID Registry V2 contract
  --l2-key-registry-v2-address <address>  The address of the L2 Openrealm Key Registry V2 contract
  --l2-storage-registry-address <address> The address of the L2 Openrealm Storage Registry contract
  --l2-resync-events                      Resync events from the L2 Openrealm contracts before starting (default: disabled)
  --l2-clear-events                       Deletes all L2 events before starting (default: disabled)
  --l2-first-block <number>               The block number to begin syncing events from L2 Openrealm contracts
  --l2-chunk-size <number>                The number of events to fetch from L2 Openrealm contracts at a time
  --l2-chain-id <number>                  The chain ID of the L2 Openrealm contracts are deployed to
...
Debugging Options:  
  ...
  --resync-name-events                  Resync events from the Qname server before starting (default: disabled)
  --chunk-size <number>                 The number of blocks to batch when syncing historical events from Openrealm contracts. (default: 10000)
...
Options:
  -s, --server <url>  Openrealm RPC server address:port to connect to (eg. 127.0.0.1:2283) (default: "127.0.0.1:2283")
